...
DBG_STATIC int webserver_get_info(const char *filename, struct File_Info *info)
{
	int result = -1;
...
DBG_STATIC int webserver_read(UpnpWebFileHandle fh, char *buf, size_t buflen)
{
...
DBG_STATIC int webserver_write(UpnpWebFileHandle fh, char *buf, size_t buflen)
{
	return -1;
}

DBG_STATIC int webserver_seek(UpnpWebFileHandle fh, off_t offset, int origin)
{
	WebServerFile *file = (WebServerFile *) fh;
	off_t newpos = -1;
	int result = -1;
...
DBG_STATIC int webserver_close(UpnpWebFileHandle fh)
{
	WebServerFile *file = (WebServerFile *) fh;
